...
const MapInfo = () => {
  ...
  const maps = [customMap, factoryMap, interchangeMap, labMap, lightHouseMap, reserveMap, shorelineMap, streetsOfTarkovMap, woodMap];
      ...
      <div className="map-container">
        {bgMaps.map((bgMap, index) => (
          <div className="map-wrapper" key={index}>
            <img src={selectedMap === bgMap.text ? maps[index] : bgMap.img} alt={bgMap} onClick={() => onClickBgImg(bgMap.text)} />
            <div className="map-overlay">{bgMap.text}</div>
          </div>
        ))}
      </div>
      {isModalOpen && <MapModal mapSrc={maps[bgMaps.findIndex((bgMap) => bgMap.text === selectedMap)]} altText={selectedMap} closeModal={closeModal} />}
    </div>
  );
};
